...
| Telegram enrollment, polling, inbox/outbox and rendering | Partially implemented with fakes | Shared task-control contracts, SQLite migrations 16-17, settings-backed capability, fake actor enrollment for tests, durable fake inbox/cursor records and a durable outbox queue exist. No live Telegram client, long-poll loop, real setup/pairing, real device enrollment or renderer is implemented. |
| Remote actor authorization and command receipts | Partially implemented with fakes | Opaque action references, fake callback validation and idempotent receipts exist for personal Save answer and Answer and resume. Real Telegram pairing, Bot API callback receipting and shared-command publication are not implemented. |
...
| --- | --- | --- | --- | --- | --- | --- |
| RTC-01 | Feature gates and setup state | Users can enable notifications/control intentionally; disabled integrations reject queued remote starts with clear status. | D01, D03, I01, B01 | Partial: shared capability status, default-off settings, a capability API endpoint and settings-page controls exist. Still add real pairing/setup UI before live use. | Current evidence: disabled-by-default capability test, dynamic settings-backed capability test and disabled remote-action rejection. Remaining: setup/pairing UX and local Stop visibility. | M1 |
| RTC-02 | Telegram pairing and roster identity | Only enrolled Telegram users/bots can act; usernames are labels only. | D02, D04, I01, B02-B03 | Partial with fakes: actor records and wrong-actor validation exist. Still add pairing challenges, bot/device/person replacement flows and audit fields. Depends on RTC-01. | Current evidence: fake wrong-actor rejection. Remaining: replay, expiry, wrong chat/topic/bot/device replacement coverage. | M1 |
| RTC-03 | Durable Telegram inbox/outbox | Offline/retry behaviour is visible and no update is lost before offset advancement. | D15, I03, B07 | Partial with fakes: durable outbox rows, failed-send state, durable inbox rows, polling cursor and fakeable Bot API adapter exist. Still add live long-poll loop, throttling/backoff policy and callback dispatch. Depends on RTC-02. | Current evidence: fake send timeout/failure/retry state, duplicate update handling and offset advancement after persistence. Remaining: throttling, reconnect gap handling and live Bot API disabled state. | M1 |
| RTC-04 | Personal task action routing | A Telegram answer maps to the current question and can Save answer or Answer and resume once. | D11, I07-I08, B08-B12 | Partial with fakes: callbacks route into `humanInput` through action validation; legacy local API remains separate. Still add real Bot API callback handling and setup authority. Depends on RTC-03. | Current evidence: fixture task saves, reissues current actions, resumes once; duplicate and stale callbacks rejected. Remaining: live transport disabled until setup/gates. | M1 |
| RTC-05 | Task status rendering | Phone status separates execution, decision and receipt state without leaking raw logs. | D13, I12, B05, B07 | Add sanitized renderers and status age model; update existing local task views only where needed. | Snapshot tests for status copy and no localhost/raw transcript/secret content. | M1 |
