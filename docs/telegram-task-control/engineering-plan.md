...
| Telegram enrollment, polling, inbox/outbox and rendering | Partially implemented with fakes | Shared task-control contracts, SQLite migration 16, fake actor enrollment for tests and a durable outbox queue exist. No live Telegram client, polling loop, setup UI, real device enrollment or renderer is implemented. |
...
| --- | --- | --- | --- | --- | --- | --- |
| RTC-01 | Feature gates and setup state | Users can enable notifications/control intentionally; disabled integrations reject queued remote starts with clear status. | D01, D03, I01, B01 | Partial: shared capability status and a default-off API endpoint exist. Still add local settings and setup UI before real use. | Current evidence: disabled-by-default capability test and disabled remote-action rejection. Remaining: setup UI/settings and local Stop visibility. | M1 |
...
| RTC-03 | Durable Telegram inbox/outbox | Offline/retry behaviour is visible and no update is lost before offset advancement. | D15, I03, B07 | Partial with fakes: durable outbox rows and failed-send state exist. Still add `server/src/integrations/telegram/` with fakeable Bot API client, inbox/update store, polling and retry queue. Depends on RTC-02. | Current evidence: fake send timeout/failure state. Remaining: duplicate delivery, throttling, reconnect and offset-advance tests. | M1 |
