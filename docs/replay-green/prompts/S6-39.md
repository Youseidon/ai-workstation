...
**A — ProxyAsync + ApplyAsync both hit the mock.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Done when: `--namespace bim` ALL GREEN (109 snapshot comparisons). `GET /takeoff/converters` must already be green from S6-32; if it is the only remaining bim-namespace failure, stop and point at S6-32 rather than reimplementing it.
