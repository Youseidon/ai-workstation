# S6-20 — Native-port workPackages
...
**A — pure proxy.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.

## In scope

Namespaces: `workPackages`.
...
Prefer existing directories before new domain: `ProjectDirectory` (WorkPackageEntityType already in that type)
...
Done when: `--namespace workPackages` ALL GREEN (76 snapshot comparisons / 19 routes → 0 failed)
