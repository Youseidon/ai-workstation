# S6-29 — Native-port estimationWorkflow reads
...
**A — MapProxy (`EstimationWorkflowEndpoints.cs`).** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Done when: all 13 GET routes green; writes still 500 is OK
