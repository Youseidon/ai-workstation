# S6-33 — Native-port takeoff run, line, and scale writes
...
**A — TakeoffService + ApplyAsync both hit the mock.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Prefer existing directories before new domain: `TakeoffLineDirectory`
