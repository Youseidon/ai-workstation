# S6-41 — Native-port quotes (replace proxy + map the 9 YARP-only routes)
...
**A — two routes are `QuotesEndpoints` → bridge + `QuotesCaptureParity`. Nine routes have no MapEndpoints (`other-to-404`).** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Keep `QuotesCaptureParity` against native results if goldens still require the overlay; prove it, do not delete it speculatively. Public token routes are anonymous-by-design.
...
Done when: `--namespace quotes` ALL GREEN (45 snapshot comparisons: 7 `500-internal` + 38 `other-to-404`)
