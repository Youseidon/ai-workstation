# S6-32 — Native-port takeoff reads
...
**A — `TakeoffService` / `MockTakeoffStateBridge`.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Done when: all 17 GET routes green
