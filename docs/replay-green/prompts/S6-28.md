# S6-28 — Native-port estimator assemblies, sections, benchmarks
...
Prefer existing directories before new domain: `AssemblyPackLibrary`, `EstimateDirectory`
...
Done when: `--namespace estimator` ALL GREEN (186 snapshot comparisons / 50 routes → 0 failed). Only run the namespace gate if S6-25…S6-27 are already green; otherwise `--route` only and `continue`.
