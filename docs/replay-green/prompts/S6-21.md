# S6-21 — Native-port clientDecisions and map savedWork
...
**A — clientDecisions are MapProxy. savedWork has no MapEndpoints (other-to-404 after YARP removal).** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.

## In scope

Namespaces: `clientDecisions`, `savedWork`.
...
savedWork routes are `/win-work/saved`. They were never mapped; YARP forwarded them. Add `MapEndpoints`; do not expect a bridge.
...
Done when: both namespace gates ALL GREEN (clientDecisions 21/5 `500-internal`; savedWork 10/3 `other-to-404`)
