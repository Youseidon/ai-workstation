# S6-22 — Native-port workers, workerPortal, and map builderOrg settings
...
**A — workers/workerPortal are MapProxy. builderOrg settings have no MapEndpoints (other-to-404).** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.

## In scope

Namespaces: `workers`, `workerPortal`, `builderOrg`.
...
Identity already has `IBuilderOrgMembershipDirectory` / org settings tables. Map the two settings routes; do not invent a second org id space.
...
Done when: three namespace gates ALL GREEN (5+7+7 snapshot comparisons)
