# S6-25 — Native-port estimator core (estimates CRUD, calculate, save, match)
...
**A — `EstimatingEndpointHelpers.MapProxy`.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Prefer existing directories before new domain: `EstimateDirectory`, `EstimateLineDirectory`
...
`--namespace estimator` is 50 failing routes. This prompt owns 14. Do not start ai-runs.
...
Done when: all 14 core routes green on every scenario
