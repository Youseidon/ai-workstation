# S6-36 — Native-port takeoff quantity (including body-only delete-plan-document)
...
**A — `TakeoffQuantityEndpoints` ApplyAsync; body-only last among these routes.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Body-only is last, not a separate pile. Status already matches; port mock body oddities.
...
Done when: all 18 routes green including body-only `DELETE /takeoff/quantity/delete-plan-document`
