# S6-18 — Native-port projects writes
...
**A — pure proxy (DELETE uses `MapDeleteProject`, still the bridge).** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Prefer existing directories before new domain: `ProjectDirectory`; `IClientPortalCommercialDirectory` for comfort-pack blockers
...
Done when: `--namespace projects` ALL GREEN (81 snapshot comparisons → 0 failed). `GET /projects/{id}/cost-model` must still 404 — that is S6-44.
