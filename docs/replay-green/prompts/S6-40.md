...
**A — `TenderEndpointHelpers.MapProxy` / `MockTenderingStateBridge`.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Stage-at-issue on trade packages is a snapshot (S6-02 hazard). Port it.
...
Done when: `--namespace tender` ALL GREEN (78 snapshot comparisons / 18 routes)
