...
**A — `BimService` is a one-liner around `MockBimStateBridge`. GET handlers call `service.ProxyAsync`.** Deleting the bridge without a native handler leaves these routes returning `500 InternalError`.
...
Do not drop ApplyAsync and call it a port. Replace BimService with real reads against Postgres/Dynamo/S3 as the mock does.
...
Done when: all 10 BIM GET routes green. Skip `GET /takeoff/converters` (S6-32).
