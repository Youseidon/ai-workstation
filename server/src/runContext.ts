...
interface Entry { runId:string; workspaceId:number; promptId:number; tokenHash:string; expiresAt:number }
...
export const runContexts={
  create(runId:string,workspaceId:number,promptId:number,ttlMs=ACTIVE_TTL_MS):{token:string;tokenHash:string;expiresAt:string}{prune();const token=randomBytes(24).toString("base64url");const tokenHash=hashRunToken(token);const expiresAt=Date.now()+ttlMs;entries.set(runId,{runId,workspaceId,promptId,tokenHash,expiresAt});return{token,tokenHash,expiresAt:new Date(expiresAt).toISOString()};},
