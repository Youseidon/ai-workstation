...
interface Entry { runId:string; workspaceId:number; promptId:number|null; question:string; tokenHash:string; expiresAt:number }
...
export const runContexts={
  create(runId:string,workspaceId:number,promptId:number|null,ttlMs=ACTIVE_TTL_MS,question=""):{token:string;tokenHash:string;expiresAt:string}{prune();const token=randomBytes(24).toString("base64url");const tokenHash=hashRunToken(token);const expiresAt=Date.now()+(ttlMs??ACTIVE_TTL_MS);entries.set(runId,{runId,workspaceId,promptId,question,tokenHash,expiresAt});return{token,tokenHash,expiresAt:new Date(expiresAt).toISOString()};},
