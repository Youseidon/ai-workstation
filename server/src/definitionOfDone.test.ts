...
import { capOutput, commandPassed, resolveCommandCwd, runDodCommand } from "./dodCommands.ts";
...
test("an agent posting DONE over a failing criterion is told what was actually stored", async () => {
  // Replying "DONE" to an agent whose close was refused would be the app
  // telling exactly the kind of lie it was built to stop telling — and the
  // agent has a use for the truth: it may still have budget to fix it.
  const ctx = fixture();
  try {
    criterion(ctx.prompt.id, { kind: "COMMAND", text: "the tests pass", command: "exit 1" });
    await runDefinitionOfDoneCommands(ctx.prompt.id, null);
    ...
    const response = workspaces.updateAgentStatus(runId, {
      ...
      reason: "finished", verificationSummary: "I ran everything and it was fine",
    }) as { status: string; requestedStatus: string; definitionOfDone?: { unmet: Array<{ criterion: string }> } };

    assert.equal(response.status, "NEEDS_REVIEW");
    assert.equal(response.requestedStatus, "DONE");
    assert.equal(response.definitionOfDone?.unmet[0]?.criterion, "the tests pass");
    assert.equal(statusOf(ctx.prompt.id), "NEEDS_REVIEW");
  } finally { ctx.cleanup(); }
});

test("an item held back by an unmet definition of done is a situation a reviewer can be sent into", async () => {
  // The `dodUnmet` reviewer row shipped with nothing able to reach it: the
  // status alone does not say why an item is on NEEDS_REVIEW, so every one of
  // them looked the same. Reading the recorded cause is what makes it
  // addressable — and a genuine question from an agent still is not.
  const ctx = fixture();
  try {
    criterion(ctx.prompt.id, { kind: "COMMAND", text: "the tests pass", command: "exit 1" });
    await runDefinitionOfDoneCommands(ctx.prompt.id, null);
    workspaces.completePrompt(ctx.prompt.id, "SYSTEM", { verificationSummary: "done" });
    assert.equal(workspaces.reviewSituation(ctx.prompt.id), "dodUnmet");
  } finally { ctx.cleanup(); }
});
