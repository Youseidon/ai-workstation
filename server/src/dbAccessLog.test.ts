...
import type { DbOperation } from "./dbAccessLog.ts";
...
test("every agent endpoint is instrumented", () => {
  ...
  // Four call sites cover the five operations: the two reads are separate
  // branches, and the three writes share one POST branch that records both the
  // accepted and the refused outcome. Asserting the shape rather than a count
  // says which of those is missing when one goes.
  ...
  assert.equal((handler.match(/recordDbAccess\(/g) ?? []).length, 4);
});
