...
import { ProviderUnavailableError, startConsult, startExecute, startVerifySuite } from "../src/runService.ts";
import { WorkspaceError } from "../src/workspaces.ts";
...
test("startVerifySuite lock, persist, launch, and finish happen in order", () => {
  const body = functionBody(readFileSync(new URL("../src/runService.ts", import.meta.url), "utf8"), "startVerifySuite");
