...
import { consultWorkspaceMarkdown, contextMarkdown } from "../src/agentContext.ts";
import type { AgentPromptContext } from "../src/workspaces.ts";

function fixture(overrides: Partial<AgentPromptContext> = {}): AgentPromptContext {
  return {
    workspace: {
      id: 1, name: "Example", workDirectory: "/tmp", description: "Shared rules",
      agentsMd: "", claudeMd: "",
    },
    ...
    prompt: {
      ...
      sortOrder: 0, createdAt: "", updatedAt: "", externalKey: "S4-02",
      status: "TODO", completedAt: null, result: "", isGate: false,
    ...
    stoppedRemarks: [
      {
        id: 1, promptId: 2, runId: "prior", kind: "BLOCKER",
        content: "Need a payment decision.", actorType: "AGENT", createdAt: "2026-08-28T00:00:00.000Z",
      },
      {
        id: 2, promptId: 2, runId: null, kind: "HUMAN_RESPONSE",
        content: "Use Stripe test mode.", actorType: "USER", createdAt: "2026-08-28T00:01:00.000Z",
      },
    ],
    dependencies: [{ externalKey: "S4-01", title: "Cart", status: "DONE", result: "33/33 green" }],
    gate: null,
...
test("run context tokens are unique, scoped, revocable, and expire", async () => {
  ...
  runContexts.revoke("run-1");
  assert.equal(runContexts.authenticate("run-1", first.token), null);
  assert.notEqual(runContexts.authenticate("run-2", second.token), null);
  const expiring = runContexts.create("run-3", 5, 6, 1);
  await new Promise((resolve) => setTimeout(resolve, 5));
  assert.equal(runContexts.authenticate("run-3", expiring.token), null);
  runContexts.revoke("run-2");
});

test("agent context is composed from records without directing the agent to prompt files", () => {
  const markdown = contextMarkdown(fixture(), "execute");
  ...
  assert.match(markdown, /## How this run ends/);
  assert.match(markdown, /Post exactly one of/);
  assert.match(markdown, /remaining work is never a blocker/);
  ...
  assert.doesNotMatch(markdown, /execution window/);
});

test("consult context has a live-tree banner and no completion protocol", () => {
  const markdown = contextMarkdown(fixture({
    prompt: {
      id: 2, suiteId: 1, title: "Checkout", content: "Implement checkout.",
      sortOrder: 0, createdAt: "", updatedAt: "", externalKey: "S4-02",
      status: "BLOCKED", completedAt: null, result: "Need a payment decision.",
      isGate: false, parentPromptId: null, childOrder: 0,
    },
    stoppedRemarks: [],
    history: { remarks: [], events: [] },
  ...
  assert.doesNotMatch(markdown, /How this run ends/);
