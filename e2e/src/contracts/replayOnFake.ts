...
export async function replayOnFake(): Promise<Replayed> {
  ...
  try {
    ...
    const sent = await call("sendMessage.withKeyboard", "sendMessage", { chat_id: chatId, text: "Contract recording: question card", link_preview_options: { is_disabled: true }, reply_markup: { inline_keyboard: [[{ text: "Record tap", callback_data: "tc_contractrecording00000" }]] } });
    ...
    const updates = polled.result as Array<{ update_id: number; message?: { reply_to_message?: unknown }; callback_query?: unknown }>;
    const at = (predicate: (update: (typeof updates)[number]) => boolean) => updates.find(predicate);
    steps["update.messageReply"] = { status: 200, body: at((update) => Boolean(update.message?.reply_to_message)), omittable: [] };
    steps["update.messagePlain"] = { status: 200, body: at((update) => Boolean(update.message) && !update.message?.reply_to_message), omittable: [] };
    steps["update.callbackQuery"] = { status: 200, body: at((update) => Boolean(update.callback_query)), omittable: [] };
