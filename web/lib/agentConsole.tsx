...
interface AgentConsoleApi extends ConsoleState {
  /**
   * The execute writer for single-run UI. Consults do not take this slot —
   * they have a separate briefing pane.
   */
  run: RunStatus | null;
...
export function AgentConsoleProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AgentConsoleApi>(() => {
    const run = state.runs.find((entry) => entry.role === "execute") ?? null;
    return {
      ...state,
      run,
