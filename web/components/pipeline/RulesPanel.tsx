...
import { COOLING_MINUTES, PROVIDER_IDS, TRANSIENT_PATTERNS } from "@agent-console/shared";
...
function SuiteFallbackEditor({
  ...
  async function save(next: ProviderId[]) {
    ...
    setBusy(true);
    try {
      await workspaceApi.updatePipelineDefaults(SERVER_URL, pipelineId, suiteId, { defaultFallbackProviders: next });
      onChanged?.();
    } finally {
      setBusy(false);
    }
